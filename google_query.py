...
def query(question):
    ...
    buffer = StringIO()
    c = pycurl.Curl()
    #c.setopt(c.URL, "https://www.google.co.kr/search?q="+encoded_question+"&hl=en")
    c.setopt(c.URL, "https://www.google.co.kr/search?q="+encoded_question+"&hl=ko")
...
def queryAndDump(question):
    ...
    c = pycurl.Curl()
    #c.setopt(c.URL, "https://www.google.co.kr/search?q="+encoded_question+"&hl=ko")
    c.setopt(c.URL, "https://www.google.co.kr/search?q="+encoded_question)
    c.setopt(c.USERAGENT, "Mozilla/5.0 (X11; Linux x86_64; rv:17.0) Gecko/20121202 Firefox/17.0 Iceweasel/17.0.1")
    c.setopt(c.WRITEDATA, buffer)
    c.perform()
    c.close()
